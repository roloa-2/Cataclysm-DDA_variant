...
作ったことと、これから作りたい事が混じった、制作の現状整理も兼ねた新機能の詳細リストです。
...
- 世界設定のオプションから排泄要素を有効にできる
  - TODO 今はデフォルトでオフだけど、やっぱデフォルトでオンにしとこう
- 画面配置の都合上、排泄欲求のステータス画面への表示は「ラベル」のみ
...
- 排泄行動はいつでも可能で、10分間かかる
  - TODO ＠さんトイレ長すぎかも、要調整
- 排泄行動が完了すると、「排泄量」「排泄欲」をゼロにして、「排泄量」に応じた量の排泄物を足元に落とし、意欲がアップする
  - トイレや車載トイレの地形上で排泄すると、さらに意欲にボーナス
  - 排泄完了時に、所持品の中の「紙製かつ、容積が0.1L～0.5Lかつ、不潔でない」アイテムをトイレットペーパーとして\*使用\*できる
    ...
    - アイテムのjsonに\*使用\*時の意欲とメッセージを指定できる
      - デフォルトではVibrator_Junkyardから札束や本物のトイレットペーパーあたりに指定してます
      - TODO 意欲が指定されてないアイテムを使うと意欲にマイナス21億のペナルティを受けるバグを直す
  - ウォシュレット付きトイレの上で排泄した場合、ウォシュレットを使える
    - 車の水1L、バッテリー500、1分で意欲が上がる
- 排泄しないまま「排泄欲」が上限を超えると漏らす
  - 漏らすと、即時・強制的に排泄すると同時に下半身の装備をすべて不潔にして、意欲が大幅にダウンする
...
- 愛玩用リトルメイド(搾乳可能)からクラフトで、愛玩用リトルメイド(散歩)を作って使用することで、愛玩用リトルメイドを友好Mobとして召喚できる
- 友好Mobのメイドさん(ショゴス)か、愛玩用メイドをeで調べることでペットメニューが開く
  - TODO あまりない状況だけど敵対的なメイドに対しても開けるかも
  - TODO 愛玩用でない通常のリトルメイドも対象にしたい？
